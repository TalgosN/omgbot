...
def kpi_help_text():
    ...
    lines = [
        ...
        '',
        '<b>#серт</b> — номер и сумма продажи',
        '<b>#абик</b> — номер и сумма продажи',
        '<code>#серт 1234 1800</code>',
    ...
    if rules:
        ...
        for rule in rules:
            ...
            elif rule.get('type') == 'reported_percent':
                hint = escape(str(rule.get('messageFormat') or 'номер и сумма'))
            else:
                hint = 'комментарий'
...
def help_handler(message, bot):
    ...
    if message.text == '🚀 Быстрый старт':
        text = (
            ...
            '📋 <b>Задачи смены</b> — открыть в модуле OMG Shift и отправить отчёт.\n'
