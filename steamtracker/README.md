# Steam Tracker v2: лицензии, каталог и подготовка промо

## Актуальная модель управления

Единственный источник рабочих данных — `db/steamtracker_v2.db`.
Управление выполняется через
`Главное меню → 🎮 Steam Tracker`:

- сотрудники ищут и фильтруют активные игры, просматривают карточки и
  наличие лицензий;
- менеджеры управляют промо и каталогом игр;
- владелец дополнительно управляет Steam-аккаунтами, расписанием,
  синхронизацией и системными настройками;
- Google Sheets получает отчётные копии и больше не применяется обратно;
- каталог содержит отчёт отсутствующих лицензий по клубам и ПК только для
  активных игр;
- команда `sync-catalog-sheets` оставлена только как понятное сообщение
  об отключённом старом процессе и не изменяет SQLite.

При первой загрузке новой версии существующие согласованные игры
автоматически становятся управляемыми активными играми. Прочие приложения,
найденные в библиотеках Steam, не попадают в менеджерский каталог.
Новую игру можно активировать только после обнаружения её лицензии хотя бы
на одном активном Steam-аккаунте. Если исчезает последняя лицензия, активная
игра автоматически переводится на паузу.
...
- три отдельных текста: сотрудникам, Telegram и VK;
- согласование и идемпотентный outbox с отключаемой доставкой сотрудникам;
- Steam Store-описания, жанры, категории и изображения для 89 игр;
- обложка, суммарные часы по активным клубным аккаунтам и место по
  популярности в карточке игры;
- отдельный read-only каталог для обычных сотрудников;
- генерация отдельного менеджерского описания из карточки игры с
  предпросмотром и явным сохранением;
...
- административный раздел `Steam Tracker → 📣 Промо`;
...
После согласования создаются три записи outbox. По умолчанию все получают
статус `ready_dry_run`. Если включена доставка сотрудникам, сотруднический
текст отправляется в рабочий Telegram-чат и получает статус `sent`, а
Telegram и VK остаются в `ready_dry_run`. Повторное согласование не создаёт
дубли.
...
откладывание. При включённом `EMPLOYEE_DELIVERY_ENABLED` согласование
отправляет сотруднический текст в рабочий чат; Telegram и VK остаются в
dry-run.
...
```text
Главное меню → Steam Tracker → 📣 Промо
```
...
## Разовое заполнение описаний каталога

Отдельный OpenRouter-промпт готовит русское описание для сотрудника и блок
«Кому рекомендовать» без скидок, акций и социальных текстов. Первый этап
только создаёт проверяемый JSON:

```powershell
python steamtracker_cli.py generate-manager-descriptions --limit 5
python steamtracker_cli.py generate-manager-descriptions --resume --workers 3
```

Файл сохраняется в
`Reports/steamtracker_manager_descriptions.json`. Для конкретных игр можно
несколько раз указать `--app-id`. Параметр `--workers` принимает значение
от 1 до 5; по умолчанию запросы выполняются последовательно.

После ручной проверки:

```powershell
python steamtracker_cli.py apply-manager-descriptions `
  --confirm APPLY_MANAGER_DESCRIPTIONS
```

Применение не перезаписывает существующие менеджерские описания и отклоняет
запись, если после генерации изменились название, статус или исходные
Steam-данные игры.
...
- отправка сотрудникам выключена по умолчанию и включается отдельным
  серверным флагом;
- публикация в Telegram и VK не реализована;
