...
| `bronix.py` | синхронизация Bronix, локальная история броней и уведомления |
...
| каждые 5 минут, `08:00-23:59` | быстрое обновление броней Bronix на ближайшие 7 дней |
| `08:00` | глубокая синхронизация Bronix: последние 30 дней и будущий год |
| `08:20` | контрольное обновление Bronix перед уведомлением |
| `08:30` | уведомление `CHAT_CALLCENTER` из свежей локальной базы броней |
...
| `CHAT_CALLCENTER` | группа Коллцентра для уведомлений Bronix | для Bronix |
| `BRONIX_BOT_API_URL` | endpoint чтения броней Bronix | для Bronix |
| `BRONIX_BOT_API_TOKEN` | Bearer-токен Bronix | для Bronix |
| `BRONIX_HISTORY_START` | начало первичного импорта, по умолчанию `2019-01-01` | — |
...
Если токен Bronix не заполнен, только синхронизация и проверка бронирований
пропускается, остальные функции бота продолжают работать.

### Неоплаченные брони Bronix
...
диапазонами загружает всю историю с `BRONIX_HISTORY_START`.
Уведомление в `08:30` только читает SQLite и не отправляется, если последнее
успешное обновление старше 15 минут. API Bronix читается страницами и
автоматически делится на разрешённые диапазоны. Текущее состояние хранится в
`booking_orders`, изменения оплаты, статуса, даты, формата и количества гостей
- в `booking_order_history`, состояние синхронизации - в `booking_sync_state`.
...
21 день с номером-ссылкой на Bronix. Роли `2-3` видят краткую сводку
сегодняшних броней по всем физическим клубам.

При первом запуске существующие записи прежней системы переносятся в
универсальные таблицы как резервные. Они остаются доступными до успешного
полного импорта Bronix. После завершения импорта резервные записи исключаются
из рабочих выборок, поэтому дубли в Mini App и рассылках не появляются.

Первичный импорт можно запустить вручную после настройки доступа:

```bash
docker compose exec bot python bronix.py
```
...
```bash
docker compose exec bot python bronix.py --full
```

Роли `2-3` могут вручную проверить ту же рассылку, которая выполняется в
`08:30`, командой `/bronix_test`.
