...
Рабочий источник настроек находится в таблице [Виарыч](https://docs.google.com/spreadsheets/d/1LxBCPpWXtpS_EVhGUNuH2k4HtPnsu53ZF-4QaRET08Q/edit). Для синхронизации в ней должны существовать четыре листа:
...
Бот не читает старые листы `Settings`, `Tags` и `Questions`, их можно архивировать или удалить вручную. Конфигурационные листы автоматически не создаются и не восстанавливаются: при отсутствии любого из них синхронизация завершится ошибкой. Изменения применяются кнопкой `Обновить настройки`: бот сначала полностью проверяет таблицу, затем атомарно сохраняет `data/clubs.json`, создаёт `data/clubs.json.bak` и сразу переключает работающий процесс на новую версию. При ошибке локальный файл и активная конфигурация не меняются.
...
`AccountName` хранит форму названия для текстов бота, а `ShiftName` должно точно совпадать с названием локации в OMG Shift. `ScheduleVisible` управляет присутствием клуба в расписании. Для видимого клуба `ScheduleEmoji` обязательно заполняется явно.
...
`.env`, база, Google-ключ и `data/*.json` не обновляются через Git. После `git pull` они остаются на сервере. Первый старт новой версии сам применит ещё не выполненные изменения схемы и добавит отсутствующие рабочие таблицы.

Миграция БД версии `1` один раз удаляет неиспользуемые legacy-таблицы `users`, `admins`, `broadcasts` и `schema_migrations`, затем переименовывает рабочие `users_new` в `users` и `broadcasts_new` в `broadcasts`. Перед первым запуском этой версии резервная копия SQLite обязательна. Успешное выполнение отмечается в `PRAGMA user_version`, поэтому повторный запуск ничего не меняет.
...
- одноразовая миграция имён рабочих таблиц;
