...
| `08:30` | проверка неоплаченных броней Bukza и уведомление `CHAT_CALLCENTER` |
...
Перед уведомлением выполняется синхронизация локальной базы. При первом запуске
бот диапазонами импортирует заказы с `BUKZA_HISTORY_START`; следующие запуски
обновляют последние 30 дней и будущий год. Ограничение Bukza в 5000 строк
обходится автоматическим дроблением периода. Текущее состояние хранится в
